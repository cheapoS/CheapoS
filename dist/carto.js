...
function settings(api,repository){
 ...
 d.innerHTML='<form><div class="modal-header"><h2>Project context</h2><button type="button" class="icon-button" data-close aria-label="Close">×</button></div><p>Carto helps agents find related code and understand dependencies. Its suggestions help agents navigate; they do not approve changes.</p><label class="checkbox-field"><input type="checkbox" name="enabled"><span>Use Carto for this project</span></label><p class="small muted">Indexes stay local in cheapoS’s cache. Each task copy refreshes changed files automatically. Agents can always inspect files normally.</p><p data-status role="status">Loading…</p><p data-install hidden>Install the optional runtime from the cheapoS repository:<br><code>python3 scripts/install_carto.py</code></p><p class="form-error" role="alert"></p><div class="modal-footer"><button type="button" class="outline-button" data-refresh>Refresh status</button><button type="button" class="outline-button" data-rebuild>Rebuild index</button><button type="submit" class="primary-button">Save</button></div></form>';
