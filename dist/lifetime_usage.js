...
const CheapOSLifetimeUsage = (() => {
  ...
  function open({dialog,api,header}){
    ...
    async function checkPending(){if(closed||pairChecking||!d.isConnected||!current?.club?.pairing_pending)return;pairChecking=true;try{const status=await api('/club/check',{});if(status.is_linked)await load();}catch(error){const output=q('[data-club-error]');if(output)output.textContent=error.message||'Unable to check approval; retry shortly.';}finally{pairChecking=false;schedulePairCheck();}}
