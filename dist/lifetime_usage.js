...
const CheapOSLifetimeUsage = (() => {
  ...
  function renderClub(club={}, viewMode='local'){
    ...
    return `<section class="club-panel"><div class="club-card-content"><h3>The cheapos Club</h3>
  ...
  function render(data, viewMode='local'){
    ...
    <div class="usage-tab-panel" data-tab-panel="overview">
      ...
      <div class="savings-highlight-card">
        ...
        <p class="savings-highlight-main"><strong>${number(zeroCostTokens)} tokens</strong> ${isRemote ? `verified on The cheapos Club scoreboard for @${escape(rp.handle)} at $0 out-of-pocket API cost.` : 'consumed at $0 out-of-pocket API cost across public-free, account-included, and local models.'}</p>
  ...
  function open({dialog,api,header,onClub}){
    ...
    <p class="small" ${onClub?'':'hidden'}>Account, sharing and sync: <button type="button" class="text-link" data-open-club>The cheapos Club →</button></p>
    ...
    async function load(){
      ...
      try{
        ...
        if(scope)scope.textContent='Local usage includes recorded requests from before sharing was enabled, plus paid and unknown access. The Club counts accepted, eligible reports across your linked installations. These totals can differ.'+
          (!hasRemote?(current.club?.is_linked?' The Club total is unavailable right now; showing local usage. Refresh totals to try again.':' Connect your Club account in The cheapos Club settings to compare totals.'):'');
  ...
  function openClub({dialog,api,header,onUpdated}){
    const d=dialog(`${header('THIS INSTALLATION','The cheapos Club')}
    ...
    function bindClubActions(){
      ...
      for(const [name,path,values] of actions){
        ...
        if(button)button.onclick=async()=>{
          ...
          try{
            const payload=['preferences','share'].includes(name)?{enabled:name==='share'||Boolean(current?.club?.sync_enabled),share_models:Boolean(q('[data-club-models]')?.checked),share_jobs:Boolean(q('[data-club-jobs]')?.checked)}:values;
