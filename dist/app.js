...
const CheapOSChatView = (() => {
  ...
  function workingMarkup(task,entry) {
    ...
    const work=task.working_states?.[key];if(!work?.steps?.length&&!work?.next_action)return '';
    const phase=task.pending_approval?'Needs approval':['paused','budget_paused','interrupted'].includes(task.status)?'Paused':task.status==='reviewing'?'In review':CheapOSGuide.isActive(task.status)?'Working':'Saved approach';
