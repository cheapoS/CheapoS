...
function executionPreferences() {
  ...
  form.onsubmit=e=>{e.preventDefault();formAction(form,async()=>{const f=new FormData(form);state.preferences=await api('/preferences',{execution:{...saved,...Object.fromEntries(['mode','local_model','local_reviewer','local_planner','coordinator_model'].map(k=>[k,String(f.get(k)||'')])),coordinator_assistance:f.get('coordinator_assistance')==='on',development_mode:f.get('development_mode')==='on'}});d.close();renderComposer();toast(state.task?'Defaults saved for new chats. This chat keeps its saved coordinator setting.':'Defaults saved for new chats.');})};
}
...
async function openConnections(afterSave, taskContext=null) {
  ...
  $('#add-gateway',d).onclick=()=>{
    ...
    type.onchange=()=>{const label=type.options[type.selectedIndex].text;if(name.value===previousName)name.value=label;previousName=label;if(url.value===previousURL){url.value=type.value==='cliproxyapi'?'http://127.0.0.1:8317/v1':type.value==='omniroute'?'http://127.0.0.1:20128/v1':'';previousURL=url.value}};
  ...
  function updateGateway() {
    ...
    const gatewayName=({omniroute:"OmniRoute",cliproxyapi:"CLIProxyAPI","9router":"9Router",litellm:"LiteLLM",compatible:"Compatible gateway"})[g.settings?.gateway_type||"omniroute"];
