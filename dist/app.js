...
const CheapOSChatView = (() => {
  ...
  function detailEvent(event,thinkingOpen=false,entryReply='') {
    ...
    if(['checks','command'].includes(event.kind)) return commandMarkup(d,{key:d.run_id||event.id,open:!d.passed});
  ...
  function eventsMarkup(events,entryReply='',suppressThinkingOpen=false) {
    const latestThinking=suppressThinkingOpen?null:events.findLast(e=>e.kind==='generation'&&e.detail?.thinking);
  ...
  function routingDetails(task) {
    ...
    const structural=(task.routing_traces||[]).flatMap(t=>(t.attempts||[]).filter(a=>a.structural_telemetry?.length).map(a=>({request:a.request_id,measurements:a.structural_telemetry})));
    const structuralMarkup=structural.length?`<details data-event="structural-diagnostics"><summary>Redacted structural telemetry</summary><p class="small muted">Local representation sizes and validation only. Upstream transformations are unknown; decoding differences do not prove corruption.</p><pre>${esc(JSON.stringify(structural,null,2))}</pre></details>`:'';

    if(!trace.rows.length&&!trace.historyNotice)return '';
    return `<details class="routing-trace" data-event="routing-log" id="routing-diagnostics"><summary>Routing &amp; request diagnostics · ${trace.rows.length} selection${trace.rows.length===1?'':'s'}</summary><p class="small muted">Newest selection and request first. These are task-wide diagnostics.</p>${trace.historyNotice?`<p class="small muted">${esc(trace.historyNotice)}</p>`:''}${[...trace.rows].reverse().map(r=>`<details data-event="route-${esc(r.id)}"><summary><strong>${esc(r.role)} · ${esc(r.selected||'No route selected')}</strong></summary><p>Requested: ${esc(r.requested)} · Selected: ${esc(r.selected||'none')}</p>${r.notice?`<p class="small muted">${esc(r.notice)}</p>`:''}<details data-event="route-candidates-${esc(r.id)}"><summary>Candidate checks (${r.candidates.length})</summary><ol>${r.candidates.map(c=>`<li>${esc(c)}</li>`).join('')}</ol></details><details data-event="route-requests-${esc(r.id)}"><summary>Requests (${r.attempts.length})</summary><ol>${[...r.attempts].reverse().map(a=>`<li>${esc(a)}</li>`).join('')}</ol></details><p>${esc(r.gateway)}</p></details>`).join('')}${structuralMarkup}</details>`;
  }
  function workingMarkup(task,entry) {
    if(task.branch_run?!entry.owner:!entry.latest)return '';
    const key=task.branch_run?.current_item_id?'item:'+task.branch_run.current_item_id:'interactive';
    const work=task.working_state||task.working_states?.[key];if(!work?.steps?.length&&!work?.next_action)return '';
    const phase=work.historical?'Earlier approach':task.pending_approval||CheapOSGuide.canCommit(task)?'Needs approval':['paused','budget_paused','interrupted'].includes(task.status)?'Paused':task.status==='reviewing'?'In review':CheapOSGuide.isActive(task.status)?'Working':'Saved approach';
...
function gatewayKeyStatus(gateway={}) {
  ...
  return 'No client key configured. Provider credentials and your dashboard password stay in your gateway.';
}
...
function renderComposer() {
  ...
  $('#composer-permissions').hidden=!(task&&state.taskPermissions?.id===task.id&&(state.taskPermissions.task_commands||state.taskPermissions.commands.length||state.taskPermissions.project_grants?.length));
  $('#composer-permissions').textContent=state.taskPermissions?.task_commands?'Task commands allowed':state.taskPermissions?.project_grants?.length?'Tests allowed this session':'Commands allowed this session';
  $('#composer-permissions').title=state.taskPermissions?.task_commands?'This task · persists across restarts':state.taskPermissions?.project_grants?.length?'This project · until cheapoS restarts':'This chat · until cheapoS restarts';
...
function renderTask({resetScroll=false}={}) {
  ...
  $('#task-status').textContent=!taskBusy(task)&&CheapOSGitWorkflow.merged(task)?'Merged on GitHub':task.branch_run?CheapOSBranchUI.projectRun(task).label:labels[task.status]||task.status;
  $('#task-date').textContent=date(task.created_at);
  $('#task-eyebrow').classList.toggle('running',taskBusy(task));
  $('#task-subtitle').textContent=!taskBusy(task)&&CheapOSGitWorkflow.merged(task)?CheapOSGitWorkflow.mergeSummary(task.pull_request).description:task.branch_run?'Unattended work on '+task.branch_run.feature_ref.replace(/^refs\/heads\//,'')+'. Final integration stays your decision.':task.demo?'Scripted models. Real edits, checks, and checkpoint reviews.':task.status==='approved'?'Ready for your decision. Approve and commit, or keep chatting.':'Working in a separate copy of your repository.';
  if(task.branch_run){
    ...
    $('#diff-tally').innerHTML=`<span>+${add}</span><span>−${remove}</span>`;
  }else{
  ...
  if(usagePill){
    ...
    usagePill.textContent=`${tokenUsageLabel(task)} · ${actualCost==null?'cost unknown':money(actualCost)}${omitProvenance?'':` · ${provenance}`}`;
...
function checkDiagnosticsMarkup(check) {
  const diagnostics=check.diagnostics;
  if(!diagnostics)return '';
  const items=Array.isArray(diagnostics.items)?diagnostics.items.slice(0,80):[];
  const status=diagnostics.limited?'Partial diagnostic index':items.length?'Recognized diagnostics':'No supported diagnostics recognized';
  return `<details class="check-diagnostics"><summary>${esc(status)} · ${items.length}</summary><p class="command-note">Best-effort index. Original output and command status remain authoritative. Locations are relative to ${esc(check.directory||'.')}.</p>${items.map(item=>`<p class="command-note"><strong>${esc(item.severity||'unknown')}</strong> <code>${esc(item.path||'Location unavailable')}${item.line?':'+esc(item.line):''}${item.column?':'+esc(item.column):''}</code> ${item.name?`<code>${esc(item.name)}</code>`:''}<br>${esc(item.message||'')}${item.text_truncated?' … (text shortened)':''}<br><span class="muted">Original bytes ${esc(item.raw_evidence?.offset??'?')}–${esc(item.raw_evidence?.end_offset??'?')}</span></p>`).join('')}</details>`;
}
function rawCheckLink(check) {
  if(!check.raw_output?.bytes||!state.task)return '';
  return `<p class="command-note"><button class="text-link" data-raw-check="${esc(check.run_id)}">Open original check output</button> · ${check.raw_output.bytes.toLocaleString()} bytes${check.raw_output.truncated?' · truncated at 64 MB':''}. Latest 8 runs retained.</p>`;
}
function commandMarkup(check,{live=false,open=false,key=check.run_id}={}) {
  const status=check.kind==='command'?(live?'Running task command':check.passed?'Task command finished':'Task command needs follow-up'):live?'Running check':({passed:'Check passed',test_failure:'Test process failed',process_timeout:'Test process timed out',task_deadline:'Task deadline reached',output_limit:'Output limit reached',user_paused:'Check paused',inputs_changed:'Verification inputs changed'}[check.outcome]||(check.passed?'Check passed':check.reason==='cancelled'?'Check stopped':'Check failed'));
...
function recoveryActionAvailable(task) {
  if(!task||taskBusy(task)||CheapOSGuide.integrationPreparation(task)||task.demo||task.archived_at||task.trashed_at||task.pending_approval||!['paused','blocked','interrupted','error','budget_paused'].includes(task.status))return false;
...
function renderChat() {
  ...
  else if(!taskBusy(task)&&CheapOSGitWorkflow.merged(task))decision=CheapOSGitWorkflow.completionMarkup(task);
...
function renderActivity() {
  ...
  $$('[data-activity-file]').forEach(b=>b.onclick=()=>{
...
function reviewRows(rows,showAll=false) {
  ...
  const keep=new Set();
  rows.forEach((r,i)=>{if(r.type!=='context')for(let j=Math.max(0,i-3);j<=Math.min(rows.length-1,i+3);j++)keep.add(j)});
  const result=[];let omitted=0;
  for(let i=0;i<rows.length;i++){if(!keep.has(i)){omitted++;continue}if(omitted){result.push({type:'gap',count:omitted});omitted=0}result.push(rows[i])}
  if(omitted)result.push({type:'gap',count:omitted});return result;
}
function patchRows(text) {
  ...
  for(let i=0;i<lines.length;i++){
    ...
    if(hunk){
      old=Number(hunk[1]);
      next=Number(hunk[2]);
      rows.push({type:'hunk',old:'',new:'',text:line});
      continue;
    }
    if(old===null&&/^(diff --git |index |--- |\+\+\+ |new file |deleted file |similarity |rename )/.test(line)){
      continue;
    }
    if(old===null||![' ','+','-'].includes(line[0])){
      if(line.trim())rows.push({type:'hunk',old:'',new:'',text:line});
      continue;
    }
    if(line[0]==='+'){
      rows.push({type:'add',old:'',new:next++,text:line.slice(1)});
    }else if(line[0]==='-'){
      rows.push({type:'remove',old:old++,new:'',text:line.slice(1)});
    }else{
      rows.push({type:'context',old:old++,new:next++,text:line.slice(1)});
    }
  }
  return rows;
}
function renderChanges() {
  ...
  const files=task.changes,mergedPR=CheapOSGitWorkflow.merged(task);
  ...
  let changeIndex=-1;
  const changes=[...view.querySelectorAll('.diff-line.add,.diff-line.remove')].filter(e=>!e.previousElementSibling?.matches('.diff-line.add,.diff-line.remove'));
  const jump=delta=>{if(!changes.length)return;changeIndex=changeIndex<0?(delta>0?0:changes.length-1):(changeIndex+delta+changes.length)%changes.length;changes[changeIndex].scrollIntoView({block:'center',behavior:'smooth'})};
  const prevBtn=view.querySelector('[data-prev-change]');if(prevBtn)prevBtn.onclick=()=>jump(-1);
...
function commitDecisionMarkup(task) {
  ...
  if(CheapOSGitWorkflow.enabled(task))return CheapOSGitWorkflow.markup(task);
...
async function ensureCommitPreview(task,force=false) {
  ...
  } catch(e){entry.error=e.message;entry.code=e.code;entry.files=e.files;try{entry.integration_readiness=await api('/tasks/'+task.id+'/integration-readiness')}catch(detail){entry.error+=' '+detail.message}}
...
function bindTerminalCopy() {
  $$('[data-raw-check]').forEach(btn=>btn.onclick=async()=>{
    ...
    try{
      ...
      dialog(`${modalHeader('VERIFICATION EVIDENCE','Original check output')}<p>Unfiltered retained bytes decoded as UTF-8. At most 64 MB; latest 8 runs. See the check result for truncation and exit status.</p><pre class="raw-check-output" style="max-height:60vh;overflow:auto;white-space:pre-wrap;overflow-wrap:anywhere">${esc(text)}</pre><a href="${url}" download="check-output.log">Download original bytes</a>`,'project-modal setup-modal');
    }catch(error){toast(error.message)}
  });
...
function tokenReservationDetails(task) {
  ...
  return `<details class="token-reservations" data-event="token-reservations-${esc(task.id)}"><summary>Token accounting</summary>
    ...
    ${a.requests.length?`<ul class="reservation-list">${a.requests.map(r=>`<li><strong>${esc(r.model)}</strong><p class="small">${esc(r.role)} · ${esc(r.purpose)} · ${r.status==='pending'?'awaiting usage':'usage unconfirmed'}${r.error_code?` · ${esc(r.error_code)}`:''}</p><p>${count(r.prompt_tokens)} prompt estimate + ${count(r.output_tokens)} output estimate = <strong>${count(r.tokens)} reserved</strong></p>${r.prompt_bytes!=null?`<p class="small muted">Prompt estimate: ${count(r.prompt_bytes)} bytes + ${count(r.buffer_tokens)} buffer.</p>`:''}</li>`).join('')}</ul>`:'<p class="small muted">No retained reservations found in available request records.</p>'}
    ${a.omitted_requests?`<p class="small muted">Showing the latest 50 of ${count(a.reservation_count)} reservations. All retained reservations are included in the total.</p>`:''}</details>`;
}
function sessionJourney(t){
...
function sessionOverview(task) {
  ...
  const label=merged?'Merged':run?CheapOSBranchUI.projectRun(task).label:labels[task.status]||task.status;
  const files=run?.readiness?.manifest?CheapOSBranchUI.reviewFiles(run.readiness).length:(task.changes||[]).length;
  const checks=task.checks||[],last=task.checkpoints?.at(-1),final=run?.readiness?.review;
  const ready=run?['ready_for_merge','merged','left_on_branch'].includes(run.status):['approved','completed'].includes(task.status);
  const review=task.status==='reviewing'?'In progress':ready&&!busy&&(final?.decision==='APPROVE'||last?.decision==='APPROVE')?'Approved':!busy&&last?.decision==='REQUEST_CHANGES'?'Changes requested':files||last?'Pending':'Not requested';
  const check=checks.length?checks.at(-1).passed===true?'Passed':checks.at(-1).passed===false?'Failed':'Incomplete':'Not run';
  const tone=merged||ready&&!busy?'success':['paused','error','blocked','waiting_approval'].includes(task.status)||['paused','blocked'].includes(run?.status)?'attention':busy?'active':'idle';
  return `<section class="session-overview" aria-label="Current task"><div class="session-status ${tone}"><span class="status-dot" aria-hidden="true"></span><strong>${esc(label)}</strong></div><dl class="session-facts"><div><dt>Changed files</dt><dd>${files}</dd></div><div><dt>Latest check</dt><dd>${check}</dd></div><div><dt>Independent review</dt><dd>${esc(review)}</dd></div></dl><div class="session-overview-actions">${files?`<button class="subtle-button" id="session-changes">${ready&&!merged?'Review changes':'View changes'}</button>`:''}${checks.length?'<button class="text-link" id="session-tests">Checks</button>':''}</div></section>`;
}
function renderInspector() {
  const t=state.task,config=t&&!t.demo?t.providers:state.config;
  const scroll=$('#session-details'),scrollTop=scroll.scrollTop;
  const expanded=new Map($$('details[data-session]',scroll).map(d=>[d.dataset.session,d.open]));
  const focused=scroll.contains(document.activeElement)?document.activeElement:null;
  const focusGroup=focused?.tagName==='SUMMARY'?focused.parentElement.dataset.session:null,focusId=focused?.id;
  const group=(id,label,body,open=false)=>`<details class="session-section" data-session="${id}" ${(expanded.get(id)??open)?'open':''}><summary>${icon('chevron')}<span>${label}</span></summary><div class="session-section-body">${body}</div></details>`;
  const roles=['coordinator','planner','worker','reviewer'].filter(role=>!['coordinator','planner'].includes(role)||config[role]||t?.usage?.[role]?.tokens||t?.request_metrics?.some(r=>r.role===role)).map(role=>`<div class="role-row"><span class="role-icon ${role}-icon">${icon(role==='worker'?'code':'spark')}</span><span><span class="role-label">${role.toUpperCase()}</span><strong>${esc(t?.demo?'Scripted demo':(role==='planner'?(t?.request_metrics?.findLast(r=>r.role==='planner')?.served_model||t?.request_metrics?.findLast(r=>r.role==='planner')?.model):null)||config[role]?.model||'Choose a model')}</strong></span></div>`);
  ...
  const tools=t?'<div class="session-tools"><button class="subtle-button" id="copy-task-json">Copy task JSON</button><button class="subtle-button" id="session-permissions">Session permissions</button><button class="subtle-button" id="open-activity">Activity log</button></div>':'';
  scroll.innerHTML=t?sessionOverview(t)+group('models','Agent crew',models,true)+group('usage',`Usage & limits <small>${money(t.usage.cost)}</small>`,usage)+group('actions','Session activity',journey)+group('workspace','Workspace & permissions',workspace+tools):intro+models+workspace;
  if(focusGroup)$$('details[data-session]',scroll).find(d=>d.dataset.session===focusGroup)?.querySelector('summary')?.focus({preventScroll:true});
  else if(focusId)document.getElementById(focusId)?.focus({preventScroll:true});
  scroll.scrollTop=scrollTop;
  if($('#session-changes'))$('#session-changes').onclick=()=>setView('changes');
  if($('#session-tests'))$('#session-tests').onclick=()=>setView('tests');
  if($('#open-activity'))$('#open-activity').onclick=()=>setView('activity');
...
const numberField=(name,label,value,min,max,step='1')=>`<label>${label}<input name="${name}" type="number" min="${min}" ${max==null?'':`max="${max}"`} step="${step}" value="${value}" required></label>`;
function workLimits(task,defaults={}) {
  const limits=task?.limits||defaults,plan=task?.branch_run?.plan;
  return {...limits,uncapped_work:plan?(plan.uncapped_work===true||plan.measurement===true):limits.uncapped_work===true};
}
function uncappedField(limits) {
  return `<label class="checkbox-field"><input type="checkbox" name="uncapped_work" ${limits.uncapped_work?'checked':''}><span>Uncapped work · ∞</span></label><p class="small muted">No turn, iteration, request, reviewer-token or working-time cap. Usage is still counted. Spending limits, test permissions, independent review and stalled-work recovery stay in force. Per-command timeouts and per-response output limits remain separate.</p>`;
}
function limitFields(limits={dollars:0,reviewer_tokens:200000,iterations:5,worker_turns:40,output_tokens:2048,run_minutes:15}) {
  ...
  return `${uncappedField(limits)}<div class="field-grid"><label class="checkbox-field"><input type="checkbox" name="free_only" ${limits.dollars===0?'checked':''}><span>Free only</span></label><div data-spending-cap ${limits.dollars===0?'hidden':''}>${numberField('dollars','Explicit spending cap ($)',limits.dollars,limits.dollars===0?0:.01,100,'0.01')}</div><label>Working time<select name="work_preset">${[['interactive','Standard · 15 minutes'],['extended','Extended · 45 minutes'],['custom','Custom']].map(([value,label])=>`<option value="${value}" ${preset===value?'selected':''}>${label}</option>`).join('')}</select><small data-work-duration>${limits.run_minutes??15} minutes of working time per run</small></label></div><details class="advanced"><summary>Advanced limits</summary><p class="small muted">Working time covers the whole run, including cooldown waits. A verification timeout bounds one command and cannot extend the run.</p><div class="field-grid">${numberField('reviewer_tokens','Reviewer token limit',limits.reviewer_tokens,512,1000000)}${numberField('iterations','Max worker iterations',limits.iterations,1,20)}${numberField('worker_turns','Worker turns per request',limits.worker_turns,1,null)}${numberField('output_tokens','Output tokens per request',limits.output_tokens,128,16384)}${numberField('checkpoint_turns','Checkpoint interval (turns)',limits.checkpoint_turns??12,2,200)}${numberField('run_minutes','Minutes per run (excludes approval waits)',limits.run_minutes??15,1,720)}${numberField('check_seconds','Seconds per verification command',limits.check_seconds??90,1,1800)}</div></details>`;
}
function bindLimitFields(form){
  const preset=$('[name="work_preset"]',form),uncapped=$('[name="uncapped_work"]',form);
  const sync=()=>{
    const limits=readLimits(new FormData(form));
    if(preset){preset.value=CheapOSGuide.workPreset(limits);preset.closest('label').hidden=limits.uncapped_work;$('[data-work-duration]',form).textContent=`${limits.run_minutes} minutes of working time per run`;}
    for(const key of ['reviewer_tokens','iterations','worker_turns','checkpoint_turns','run_minutes']){const input=$(`[name="${key}"]`,form);if(input)input.closest('label').hidden=limits.uncapped_work;}
  };
  if(uncapped)uncapped.onchange=sync;
  if(preset)preset.onchange=()=>{if(preset.value==='custom'){$('details.advanced',form).open=true;return}const next=CheapOSGuide.presetLimits(readLimits(new FormData(form)),preset.value);for(const key of ['run_minutes','worker_turns','iterations'])$(`[name="${key}"]`,form).value=next[key];sync()};
  for(const input of $$('input[type="number"]',form))input.addEventListener('input',sync);
  const free=$('[name="free_only"]',form);
  if(free)free.onchange=e=>{const cap=$('[name="dollars"]',form);if(e.target.checked)cap.value=0;cap.min=e.target.checked?0:.01;$('[data-spending-cap]',form).hidden=e.target.checked;};
  sync();
}
...
function coordinatorReassessmentMarkup(task){
  ...
  const explanation=available.reuse_saved?'The saved coordinator reply now passes validation. Continue from the current files and review feedback without another coordinator call. Original usage and limits remain counted. No new prompt needed.':available.format_repair?`Ask ${esc(available.model)} once to return correctly formatted guidance for the same saved work. The original attempt and usage remain counted. No new prompt needed.`:`Use ${esc(available.model)} to reassess these saved files once, then continue only if it provides a next step. Keeps remaining limits, permissions and review requirements. No new prompt needed.`;
  return `<p>${explanation}</p><button class="primary-button" data-chat-action="coordinator-reassess" ${busy?'disabled':''}>${busy?'Requesting reassessment…':available.reuse_saved?'Continue with saved guidance':available.format_repair?'Retry coordinator format':CheapOSGuide.coordinatorStatus(task).enabled?'Reassess with coordinator':'Enable coordinator &amp; reassess'}</button>${error?`<p role="alert">${esc(error)}</p>`:''}`;
}
...
function checkCommandText(c){return CheapOSBranchUI.checkText(Array.isArray(c)?{command:c}:c);}
function operatorRecoveryFields(cap){if(cap.planning)return `<p>${esc(cap.reason)}</p>${cap.can_retry?'<button type="button" class="outline-button" data-recovery="retry">Retry planning</button>':''}${cap.can_planner?`<label class="full-field">Choose another planner<select name="recovery_planner">${(cap.planners||[]).map(m=>`<option value="${esc(m.id)}">${esc(m.label)}</option>`).join('')}</select></label><button type="button" class="primary-button" data-recovery="planner" ${cap.planners?.length?'':'disabled'}>Use planner &amp; retry</button>${cap.planners?.length?'':'<p>No eligible planner is available right now. Check connections in Models, then retry.</p>'}`:''}<p class="form-error" role="alert"></p>`;cap={...cap,...Object.fromEntries(['enable','retry','model','reviewer','revise','checks'].map(action=>['can_'+action,cap['can_'+action]??cap.actions?.includes(action)]))};return `<p>${esc(cap.reason||'Choose the next action for this saved task.')}</p>${cap.can_enable?'<button type="button" class="outline-button" data-recovery="enable">Enable development mode for this task</button><p>Removes automatic work and recovery caps. Existing spending authorization, permissions and review remain in force.</p>':''}${cap.can_retry?'<button type="button" class="outline-button" data-recovery="retry">Retry current operation</button>':''}${cap.can_model?`<label class="full-field">Worker for this saved task<select name="recovery_model">${(cap.models||[]).map(m=>`<option value="${esc(m.id)}">${esc(m.label||m.id)}</option>`).join('')}</select></label><button type="button" class="outline-button" data-recovery="model" ${(cap.models||[]).length?'':'disabled'}>Change worker &amp; continue</button>`:''}${cap.can_reviewer?`<label class="full-field">Choose reviewer<select name="recovery_reviewer">${(cap.reviewers||[]).map(m=>`<option value="${esc(m.id)}">${esc(m.label||m.id)}</option>`).join('')}</select></label>${cap.unknown_worker_history?'<p>cheapoS did not retain the identity of a historical worker. This requires an app-level provenance repair; you do not need to write another task instruction. Choosing another reviewer cannot recover that missing evidence.</p>':''}<button type="button" class="outline-button" data-recovery="reviewer" ${(cap.reviewers||[]).length&&!cap.unknown_worker_history?'':'disabled'}>Use reviewer for fresh review</button>`:''}${cap.can_revise?'<label class="full-field">Revised current item instructions<textarea name="recovery_instructions" rows="5" placeholder="Describe the exact revised instructions for this item"></textarea></label><p>Applying approves these revised instructions for the current item. Other accepted criteria, checks, spending and permissions remain in force.</p><button type="button" class="outline-button" data-recovery="revise">Approve item revision &amp; continue</button>':''}${cap.can_checks?`<label class="full-field">Required verification · one per line; use [folder] command for a component<textarea name="recovery_required_checks" rows="4">${esc((cap.required_checks||[]).map(checkCommandText).join('\n'))}</textarea></label><label class="full-field">Final verification · one command per line<textarea name="recovery_final_checks" rows="4">${esc((cap.final_checks||[]).map(checkCommandText).join('\n'))}</textarea></label><p>This changes required verification. Removed or skipped checks are not reported as passed. Existing results will be revalidated against the new requirements.</p><p>Last full-suite runtime: ${Number.isFinite(cap.full_suite_last_seconds)?esc(cap.full_suite_last_seconds)+' seconds':'unknown'}.</p><label><input type="checkbox" name="recovery_full_suite">I approve full-suite checks included in these commands</label><button type="button" class="outline-button" data-recovery="checks">Approve verification changes &amp; continue</button>`:''}<p class="form-error" role="alert"></p>`;}
async function operatorRecovery(task){
 const d=dialog(`${modalHeader('ADVANCED OPTIONS','Recovery settings for this task')}<div data-recovery-body role="status">Loading available recovery actions…</div>`,'execution-modal');
 try{const cap=await api('/tasks/'+task.id+'/operator-recovery');if(!d.isConnected)return;const body=$('[data-recovery-body]',d);body.innerHTML=operatorRecoveryFields(cap);
 $$('[data-recovery]',body).forEach(button=>button.onclick=async()=>{const action=button.dataset.recovery,model=$('[name="recovery_model"]',body)?.value,instructions=$('[name="recovery_instructions"]',body)?.value.trim();if(action==='reviewer'&&cap.unknown_worker_history){$('.form-error',body).textContent='Repair missing worker provenance before choosing a reviewer.';return;}if(action==='revise'&&!instructions){$('.form-error',body).textContent='Enter the revised item instructions before approving.';return;}$$('[data-recovery]',body).forEach(b=>b.disabled=true);try{const result=await api('/tasks/'+task.id+'/operator-recovery',{action,...(action!=='retry'?{approved:true}:{}),...(['model','reviewer','planner','revise','checks'].includes(action)?{revision_token:cap.revision_token}:{}),...(action==='model'?{model}:{}),...(action==='planner'?{model:$('[name="recovery_planner"]',body).value}:{}),...(action==='reviewer'?{model:$('[name="recovery_reviewer"]',body).value}:{}),...(action==='revise'?{instructions}:{}),...(action==='checks'?{required_checks:CheapOSBranchUI.parseChecks($('[name="recovery_required_checks"]',body).value),final_checks:CheapOSBranchUI.parseChecks($('[name="recovery_final_checks"]',body).value),...($('[name="recovery_full_suite"]',body).checked?{full_suite_approved:true}:{})}:{})});const saved=result.task;if(saved&&state.task?.id===task.id){state.task=saved;renderTask();}d.close();await refresh();}catch(e){$('.form-error',body).textContent=e.message;$$('[data-recovery]',body).forEach(b=>b.disabled=false);}});
 }catch(e){if(d.isConnected){const body=$('[data-recovery-body]',d);body.innerHTML=`<p>Could not load recovery options. Your saved task has not been changed.</p><p class="form-error">${esc(e.message)}</p><button class="outline-button" data-reload-recovery>Try again</button>`;$('[data-reload-recovery]',body).onclick=()=>{d.close();operatorRecovery(task)}};}
}
...
async function scopedSettings(scope,section='agents',project,initialRoleSelection){
 const capturedTask=state.task,repository=project||state.project?.path||capturedTask?.source;
 const options={api,initialRoleSelection,scope:scope||(capturedTask?'task':'draft'),task:capturedTask?{id:capturedTask.id,title:capturedTask.title||capturedTask.prompt}:null,project:repository,section,models:state.gatewayModels||[],connectionsList:state.gateway.connections||[],getConnections:()=>state.gateway.connections||[],
...
async function dispatchChat() {
  ...
  if(!state.project){
    ...
    openProject(()=>{
      ...
      const existing=state.composerAttachments||[],ids=new Set(existing.map(a=>a.id));
      state.composerAttachments=[...existing,...attachments.filter(a=>!ids.has(a.id))];
      $('#chat-input').value=message;saveDraft();
      if(draftKey()!==owner)clearOwnedDraft(owner,message,attachments);
      renderComposer();
    });
    return;
  }
  ...
  try {
    if(task){
      ...
      const created=await api('/tasks',{repository,prompt:message,attachments,conversational:true,...await capturedDraftSetup(repository)});
      await loadTasks();
      clearOwnedDraft(key,message,attachments);
      if(typeof renderComposerAttachments==='function')renderComposerAttachments();
...
async function steerTask(text) {
  ...
  try {
    ...
    }else{await refresh();if(state.selection===selection)toast(saved?.operator_continue?.reason||'Your update is saved for this task.');}
    if(state.selection===selection)$('#view-container').scrollTop=$('#view-container').scrollHeight;
  }
...
function resumeDialog() {
  ...
  const fields={worker_turns:['Worker turns allowed for this request',1,null,'1'],run_minutes:['Working minutes for the next run',1,720,'1'],reviewer_tokens:['Reviewer tokens allowed',512,1000000,'1'],iterations:['Worker iterations allowed',1,20,'1'],dollars:['Explicit spending cap ($)',0,100,'0.01']};
...
async function sessionPermissions() {
  ...
  try{
    ...
    const d=dialog(`${modalHeader('SESSION PERMISSIONS','Tests and commands you’ve allowed')}<h3>Task commands · ${permissions.task_commands?'allowed':'not allowed'}</h3><p>Allow setup, dependency installation, diagnostics, builds and tests from this task copy. Persists across restarts for this task only. Commands run with your account’s permissions; this is not a security sandbox. Required checks, independent review and spending limits still apply.</p><p><code>${esc(permissions.directory)}</code></p><button class="outline-button" id="task-command-permission">${permissions.task_commands?'Revoke task commands':'Allow task commands'}</button><p class="small muted">Pause a running task before changing this permission. It does not authorize deployment, publishing, credential access or edits outside this task copy.</p><h3>Local browser verification</h3><button class="outline-button" id="agent-browser-permission">Configure agent browser verification</button><h3>Additional session permissions</h3><p>The grants below expire when cheapoS restarts. Revocation affects future commands; running commands continue.</p><h3>Project tests</h3>${grants.length?grants.map(g=>`<section><p>${esc(g.profile.project)} · unittest</p><p>Test roots: ${g.profile.roots.map(root=>esc(root==='.'?'Project root (.)':root)).join(', ')}</p><button class="outline-button" data-revoke-grant="${g.id}">Revoke project tests</button></section>`).join(''):'<p>No project test grants.</p>'}<h3>Exact commands in this chat</h3>${permissions.commands.length?permissions.commands.map(c=>`<code class="approval-command">${esc(c.join(' '))}</code>`).join(''):'<p>No exact-command grants.</p>'}${permissions.commands.length?'<button class="outline-button" id="clear-session-permissions">Clear exact commands</button>':''}<p class="form-error" role="alert"></p>`);
    $('#agent-browser-permission',d).onclick=()=>CheapOSPreview.agentSettings(api,task);
    $('#task-command-permission',d).onclick=async event=>{const button=event.currentTarget;button.disabled=true;try{await api('/tasks/'+task.id+'/task-commands',{enabled:!permissions.task_commands,directory:permissions.directory});await loadTaskPermissions(task,true);d.close();toast(permissions.task_commands?'Task command permission revoked.':'Task commands allowed for this task.');await refresh()}catch(error){$('.form-error',d).textContent=error.message;button.disabled=false;}};
...
async function openConnections(afterSave, taskContext=null, settingsHost=null) {
  ...
  const gatewayTypesList = () => state.gateway?.gateway_types || [
    {id:"direct",label:"Direct API (OpenAI-compatible)",default_url:"",default_name:"Direct API"},
    ...
    {id:"9router",label:"9Router",default_url:"http://127.0.0.1:20128/v1",default_name:"9Router"},
    {id:"litellm",label:"LiteLLM",default_url:"http://127.0.0.1:4000/v1",default_name:"LiteLLM"},
    {id:"ollama",label:"Ollama",default_url:"http://127.0.0.1:11434/v1",default_name:"Ollama"},
    {id:"vllm",label:"vLLM",default_url:"http://127.0.0.1:8000/v1",default_name:"vLLM"},
    {id:"lmstudio",label:"LM Studio",default_url:"http://127.0.0.1:1234/v1",default_name:"LM Studio"},
  ...
  const d=(settingsHost?.dialog||dialog)(`${modalHeader('SHARED CONNECTIONS','APIs, gateways & local models')}<button class="outline-button" id="models-execution">Choose agents in scoped setup →</button><p class="modal-description">These connections are shared by chats. Changing connection identity or access can affect saved chats using it. Agent choices belong to Chat setup, Project overrides, or App defaults.</p>
    <div class="button-row"><label>Saved connection<select id="saved-gateway">${(gateway.connections||[]).map(g=>`<option value="${esc(g.id)}" ${g.id===gateway.selected_connection?"selected":""}>${esc(g.name)}${g.enabled?"":" · disabled"}</option>`).join("")}</select></label><button class="outline-button" id="add-gateway">Add model or provider…</button></div><p class="small muted">This dropdown selects settings to edit. Automatic mode uses only connections you allow for automatic selection when the task starts. Manual choices stay pinned.</p><form class="gateway-card" id="gateway-form"><div class="gateway-heading"><div><strong id="gateway-name">Gateway</strong><span class="gateway-badge" id="gateway-status" role="status"></span></div><a id="gateway-dashboard" class="subtle-button" href="${esc(gateway.dashboard_url||'http://127.0.0.1:20128')}" target="_blank" rel="noopener noreferrer">Open OmniRoute ↗</a></div>
      <p id="gateway-message" class="small muted"></p><label class="full-field">Model<select id="connection-model-choice"></select></label><button type="button" class="outline-button" id="connection-use-worker">Use as worker in this chat…</button><p class="small muted">This opens this chat’s setup for review. Planner and reviewer keep their choices. Unknown prices require explicit included access below.</p><p id="gateway-instance" class="small muted"></p>
      <details class="advanced"><summary id="free-pool-title">Authorized remote model pool</summary><p class="small muted">Refreshes every five minutes, including OpenRouter’s current free models. Failed models cool down for 15–60 minutes. Provider cooldowns follow the gateway’s retry time and do not count as individual model failures. A response or tool check does not prove coding quality.</p><label class="full-field model-pool-sort">Sort models<select id="model-pool-sort"><option value="catalog">Catalog order</option><option value="coding">Coding score · highest first</option><option value="agentic">Agentic score · highest first</option></select></label><p class="small muted">Benchmarks guide initial automatic choices after your preferences, compatibility and observed task results. Missing scores stay unknown and do not exclude a model. Sorting here changes only this view.</p><p class="small muted">Scores are Artificial Analysis indices supplied by the catalog. Refresh time is when cheapoS fetched them, not when the benchmark ran.</p><div id="free-model-pool" class="free-model-pool"></div></details>
      <div class="included-access"><label class="full-field">Models included in my account<textarea id="included-model-ids" rows="4" spellcheck="false" placeholder="One exact gateway model ID per line">${esc((settings.included_models||[]).join('\n'))}</textarea></label><p class="small muted">Authorize only models covered by your existing account. This applies to this gateway connection. New models and changed connections need new authorization. Included access uses a $0 marginal estimate; it is not public-free pricing or a billing receipt.</p><label class="full-field">Tool-capable models missing catalog metadata<textarea id="gateway-tool-models" rows="2" spellcheck="false">${esc((settings.tool_models||[]).join("\n"))}</textarea><small>Optional exact IDs, one per line. Declares capability only; automatic selection still checks tool calling and requires free or included access.</small></label><button type="button" class="outline-button" id="save-included-models">Save included access</button><p id="included-access-status" class="small" role="status"></p></div><div class="gateway-actions"><button type="button" class="outline-button" data-gateway-action="start">Connect / start</button><button type="button" class="subtle-button" data-gateway-action="refresh">Refresh models</button><button type="button" class="subtle-button" data-gateway-action="stop" hidden>Stop instance</button></div>
      <details class="advanced"><summary>Startup & connection settings</summary><label class="full-field">Connection name<input name="gateway_name" value="${esc(settings.name||'OmniRoute')}" required maxlength="80"></label><label class="checkbox-field"><input name="gateway_enabled" type="checkbox" ${settings.enabled!==false?'checked':''}><span>Connection enabled</span></label><label class="checkbox-field"><input name="gateway_automatic" type="checkbox" ${settings.automatic!==false?'checked':''}><span>Allow automatic selection in future chats</span></label><label class="full-field">Exact model IDs missing from the catalog<textarea name="manual_models" rows="2">${esc((settings.manual_models||[]).join('\n'))}</textarea></label><label class="full-field">Shared upstream quota groups<textarea name="quota_groups" rows="2" placeholder="provider-prefix=account-label">${esc(Object.entries(settings.quota_groups||{}).map(([k,v])=>k+'='+v).join('\n'))}</textarea><small>Optional. Use the same account label for the same upstream account in different gateways. Prefix is the part of its model ID before the first slash. Labels are not credentials.</small></label><label class="full-field">Gateway type<select name="gateway_type">${gatewayTypesList().map(t=>`<option value="${esc(t.id)}" ${t.id===(settings.gateway_type||"omniroute")?"selected":""}>${esc(t.label)}</option>`).join("")}</select><small>Start alternative gateways separately. Use their configured loopback /v1 endpoint; direct APIs use HTTPS. Switching clears this connection’s model authorizations and client key.</small></label><label class="full-field">API base URL<input name="gateway_url" type="url" value="${esc(settings.base_url)}" required></label>
        <label class="full-field">Gateway client API key · optional<input name="gateway_key" type="password" placeholder="${gateway.key_configured?'Configured · leave blank to keep':'Only if your gateway requires a client key'}" autocomplete="new-password"></label>
...
`,'connections-modal');
  ...
  $('#saved-gateway',d).onchange=async e=>{try{state.gateway=await api('/gateway/select',{connection_id:e.target.value});await loadGateway();if(settingsHost)d.dispose();else d.close();openConnections(afterSave,taskContext,settingsHost)}catch(error){toast(error.message)}};
  $('#add-gateway',d).onclick=()=>{
    const add=dialog(`<form class="gateway-add-form">${modalHeader('CONNECTION','Add model or provider')}<div class="button-row"><button type="button" data-entry="manual" aria-pressed="true">Enter manually</button><button type="button" data-entry="paste" aria-pressed="false">Paste setup text</button></div><section data-paste hidden><label>Post, documentation or API example<textarea data-setup-text rows="5" maxlength="20000" placeholder="Paste setup instructions here. Enter your API key in the key field below."></textarea></label><button type="button" data-extract>Fill connection details</button><p data-extract-status role="status"></p><select data-endpoint-choice aria-label="Extracted API addresses" hidden></select><p class="small muted">Parsed on this computer. Links are not opened and pasted commands are never run.</p></section><label>Connection type<select name="gateway_type">${gatewayTypesList().map(t=>`<option value="${esc(t.id)}" ${t.id==='direct'?'selected':''}>${esc(t.label)}</option>`).join('')}</select></label><label>Connection name<input type="text" name="name" required maxlength="80" autocomplete="off"></label><label>API base URL<input name="base_url" type="url" required placeholder="https://api.example.com/v1" spellcheck="false"></label><p class="small muted">Direct APIs require HTTPS. Local gateways use their loopback /v1 address. Review the destination before entering your key.</p><label>API key · optional<input name="api_key" type="password" autocomplete="new-password"></label>${gatewayRememberField(gateway,'remember_key',false)}<label>Exact model ID · optional<input type="text" name="model" maxlength="200" placeholder="Discover models after saving, or enter an exact API ID" spellcheck="false"></label><label class="checkbox-field"><input name="automatic" type="checkbox"><span>Allow automatic selection in future chats</span></label><p class="small muted">Saving makes no inference request. Free access is not inferred from pasted text. Refresh models after saving, then select a model for this chat.</p><p class="form-error" role="alert"></p><div class="modal-footer"><button type="button" class="outline-button" data-close>Cancel</button><button type="submit" class="primary-button">Save connection</button></div></form>`,'gateway-add-modal');
    const type=$('[name="gateway_type"]',add),name=$('[name="name"]',add),url=$('[name="base_url"]',add);
    let previousName='',previousURL='';
    type.onchange=()=>{const gt=gatewayTypesList().find(t=>t.id===type.value);if(name.value===previousName)name.value=gt?.default_name||gt?.label||'';previousName=gt?.default_name||gt?.label||'';if(url.value===previousURL)url.value=gt?.default_url||'';previousURL=gt?.default_url||'';};
    $$('[data-entry]',add).forEach(button=>button.onclick=()=>{$('[data-paste]',add).hidden=button.dataset.entry!=='paste';$$('[data-entry]',add).forEach(b=>b.setAttribute('aria-pressed',String(b===button)));});
    $('[data-extract]',add).onclick=()=>{const result=CheapOSConnectionSetup.parse($('[data-setup-text]',add).value);type.value='direct';url.value=result.base_url;name.value=result.name;$('[name="model"]',add).value=result.model;$('[data-extract-status]',add).textContent=result.message;const choices=$('[data-endpoint-choice]',add);choices.hidden=result.candidates.length<2;choices.innerHTML='<option value="">Choose an API address</option>'+result.candidates.map(v=>`<option value="${esc(v)}">${esc(v)}</option>`).join('');choices.onchange=()=>{if(choices.value){url.value=choices.value;name.value=new URL(choices.value).hostname;}};};
    const form=$('form',add);form.onsubmit=e=>{e.preventDefault();formAction(form,async()=>{const f=new FormData(form),model=String(f.get('model')).trim();state.gateway=await api('/gateway/add',{name:String(f.get('name')).trim(),gateway_type:String(f.get('gateway_type')),base_url:String(f.get('base_url')).trim(),api_key:String(f.get('api_key')).trim(),remember_key:f.has('remember_key'),manual_models:model?[model]:[],enabled:true,automatic:f.has('automatic')});await loadGateway();add.close();if(settingsHost)d.dispose();else d.close();openConnections(afterSave,taskContext,settingsHost)})};
  };
  $('#models-execution',d).onclick=()=>{if(settingsHost)settingsHost.navigate(taskContext||state.task?'task':state.project?'draft':'defaults');else{d.close();executionPreferences();}};
  let includedRevision=settings.connection_revision;
  const gatewayForm=$('#gateway-form',d);
  function updateGateway() {
    if(!d.open)return;
    const g=state.gateway||{};
    const gtMatch=gatewayTypesList().find(t=>t.id===(g.settings?.gateway_type||"omniroute"));
    ...
    $('#gateway-instance',d).textContent=g.status==='ready'?`${g.model_count} models · ${g.settings?.gateway_type==='direct'?'Direct API':g.owned?'Started by cheapoS':'Reusing an existing instance'}`:'';
    const modelChoice=$('#connection-model-choice',d),chosen=modelChoice.value;modelChoice.innerHTML=state.gatewayModels.map(m=>`<option value="${esc(m.id)}">${esc(m.id)} · ${esc(CheapOSGuide.modelAccess(m))}</option>`).join('')||'<option value="">Refresh models to choose</option>';if(state.gatewayModels.some(m=>m.id===chosen))modelChoice.value=chosen;$('#connection-use-worker',d).disabled=!modelChoice.value;
    ...
    $('#free-model-pool',d).innerHTML=CheapOSGuide.sortModelCatalog(freeModels,$('#model-pool-sort',d).value).map(m=>`<div class="pool-model"><strong>${esc(m.id)}</strong><span>${esc(CheapOSGuide.modelAccess(m))} · ${esc(CheapOSGuide.modelHealth(m))}${m.reasoning===true?' · reasoning advertised':''}</span><small class="model-benchmark-scores">${esc(CheapOSGuide.modelBenchmarks(m).scores)}</small><small>${esc(CheapOSGuide.modelBenchmarks(m).source)}</small><small>${esc(CheapOSGuide.metadataEvidence(m))}</small>${m.health?.last_error?`<small>${esc(m.health.last_error)}</small>`:''}</div>`).join('')||'<p class="small muted">No public-free or explicitly included remote models advertising tool support are listed in this catalog.</p>';
  ...
  $('#connection-use-worker',d).onclick=()=>{const model=$('#connection-model-choice',d).value;if(!model)return;const selection={strategy:'only',connection_id:state.gateway.selected_connection,model};d.close();scopedSettings(undefined,'agents',undefined,selection);};
  $('#model-pool-sort',d).onchange=updateGateway;
  ...
  $('#save-included-models',d).onclick=()=>formAction(gatewayForm,async()=>{
    ...
    state.gateway=await api('/gateway/config',{included_models,tool_models:CheapOSGuide.includedScope($('#gateway-tool-models',d).value),expected_connection_revision:includedRevision});
    $('#included-access-status',d).textContent='Included access saved for these exact IDs. No model request was made.';
    state.gateway=await api('/gateway/refresh',{});updateGateway();await loadGateway();
  });
  ...
  $('[name="gateway_type"]',d).onchange=()=>{const omni=$('[name="gateway_type"]',d).value==='omniroute';$('[name="auto_start"]',d).disabled=!omni;if(!omni)$('[name="auto_start"]',d).checked=false;};
  $('[name="gateway_type"]',d).onchange();
  gatewayForm.onsubmit=e=>{e.preventDefault();formAction(gatewayForm,async()=>{
    const f=new FormData(gatewayForm), values={name:String(f.get('gateway_name')).trim(),enabled:f.has('gateway_enabled'),automatic:f.has('gateway_automatic'),manual_models:String(f.get('manual_models')).split('\n').map(v=>v.trim()).filter(Boolean),quota_groups:Object.fromEntries(String(f.get('quota_groups')).split('\n').filter(v=>v.trim()).map(v=>{const i=v.indexOf('=');if(i<1||!v.slice(i+1).trim())throw new Error('Use provider-prefix=account-label for each quota group');return [v.slice(0,i).trim(),v.slice(i+1).trim()]})),gateway_type:String(f.get('gateway_type')),base_url:String(f.get('gateway_url')).trim(),auto_start:f.has('auto_start'),keep_running:f.has('keep_running'),remember_key:f.has('gateway_remember')},key=String(f.get('gateway_key')).trim();if(key)values.api_key=key;
...
async function resumeBranchRun(task,savedResult,approvalValues={}) {
  ...
  if(result.needs_merge_recovery){
    ...
    const form=$('form',d);form.onsubmit=e=>{e.preventDefault();formAction(form,async()=>{await CheapOSBranchUI.mergeAndPublish({api,task,values:{recover:true,approved:true},onPending:()=>{$('[type=submit]',form).textContent='Saving merge approval…';},onTask:saved=>{d.close();receiveUpdatedTask(saved);setView('chat');if(CheapOSBranchUI.projectRun(saved).merged)toast('Reviewed changes merged locally.');},refresh});});};return;
  }
