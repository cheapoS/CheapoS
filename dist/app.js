...
`,'connections-modal');
  ...
  $('#add-gateway',d).onclick=()=>{
    ...
    let previousName='CLIProxyAPI',previousURL=url.value;
    type.onchange=()=>{
      const gt=gatewayTypesList().find(t=>t.id===type.value);
      const label=gt?.label||type.options[type.selectedIndex].text;
      if(name.value===previousName)name.value=gt?.default_name||label;
      previousName=gt?.default_name||label;
      if(url.value===previousURL){
        url.value=gt?.default_url||'';
