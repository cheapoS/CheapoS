...
const CheapOSChatView = (() => {
  ...
  function message(entry,task,decision='',showTraces=false) {
    ...
    const traceDetails=trace.rows.length?`<section class="routing-trace" aria-label="Routing decisions"><h4>Routing decisions</h4><ol>${trace.rows.map(r=>`<li><strong>${esc(r.role)} · requested ${esc(r.requested)}</strong><p>Selected: ${esc(r.selected||'none')}</p><ol>${r.candidates.map(c=>`<li>${esc(c)}</li>`).join('')}</ol><ol>${r.attempts.map(a=>`<li>${esc(a)}</li>`).join('')}</ol><p>${esc(r.gateway)}</p></li>`).join('')}</ol></section>`:'';
