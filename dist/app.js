...
async function operatorRecovery(task){
 ...
 $$('[data-recovery]',body).forEach(button=>button.onclick=async()=>{const action=button.dataset.recovery,model=$('[name="recovery_model"]',body)?.value,instructions=$('[name="recovery_instructions"]',body)?.value.trim();if(action==='revise'&&!instructions){$('.form-error',body).textContent='Enter the revised item instructions before approving.';return;}$$('[data-recovery]',body).forEach(b=>b.disabled=true);try{const result=await api('/tasks/'+task.id+'/operator-recovery',{action,...(action!=='retry'?{approved:true}:{}),...(['model','revise'].includes(action)?{revision_token:cap.revision_token}:{}),...(action==='model'?{model}:{}),...(action==='revise'?{instructions}:{})});const saved=result.task;if(saved&&state.task?.id===task.id){state.task=saved;renderTask();}d.close();await refresh();}catch(e){$('.form-error',body).textContent=e.message;$$('[data-recovery]',body).forEach(b=>b.disabled=false);}});
