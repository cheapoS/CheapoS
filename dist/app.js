...
async function operatorRecovery(task){
 ...
 $$('[data-recovery]',body).forEach(button=>button.onclick=async()=>{const action=button.dataset.recovery,model=$('[name="recovery_model"]',body)?.value,instructions=$('[name="recovery_instructions"]',body)?.value.trim();if(action==='reviewer'&&cap.unknown_worker_history){$('.form-error',body).textContent='Repair missing worker provenance before choosing a reviewer.';return;}if(action==='revise'&&!instructions){$('.form-error',body).textContent='Enter the revised item instructions before approving.';return;}$$('[data-recovery]',body).forEach(b=>b.disabled=true);try{const result=await api('/tasks/'+task.id+'/operator-recovery',{action,...(action!=='retry'?{approved:true}:{}),...(['model','reviewer','planner','revise','checks'].includes(action)?{revision_token:cap.revision_token}:{}),...(action==='model'?{model}:{}),...(action==='planner'?{model:$('[name="recovery_planner"]',body).value}:{}),...(action==='reviewer'?{model:$('[name="recovery_reviewer"]',body).value}:{}),...(action==='revise'?{instructions}:{}),...(action==='checks'?{required_checks:$('[name="recovery_required_checks"]',body).value.split('\n').map(s=>s.trim()).filter(Boolean),final_checks:$('[name="recovery_final_checks"]',body).value.split('\n').map(s=>s.trim()).filter(Boolean),...($('[name="recovery_full_suite"]',body).checked?{full_suite_approved:true}:{})}:{})});const saved=result.task;if(saved&&state.task?.id===task.id){state.task=saved;renderTask();}d.close();await refresh();}catch(e){$('.form-error',body).textContent=e.message;$$('[data-recovery]',body).forEach(b=>b.disabled=false);}});
