...
function operatorContinuationMarkup(task){const c=task.operator_continue;if(!c||(['running','interrupting'].includes(c.status)&&!['running','reviewing','verifying','queued','starting'].includes(task.status)))return '';const title={running:'Correction applied · continuing',interrupting:'Correction received · stopping the previous approach',needs_consent:'Correction saved · permission required',blocked:'Correction saved · continuation blocked'}[c.status]||'Correction saved';return `<section class="execution-notice" role="status"><strong>${title}</strong><p>${esc(c.reason||'Your instruction remains attached to this task.')}</p>${c.status==='needs_consent'?'<button class="outline-button" data-chat-action="resume">Review required permission</button>':''}</section>`;}
