...
function openConnections(afterSave, taskContext=null) {
  ...
  function updateGateway() {
    ...
    $('#free-model-pool',d).innerHTML=freeModels.map(m=>`<div class="pool-model"><strong>${esc(m.id)}</strong><span>${esc(CheapOSGuide.modelAccess(m))} · ${esc(CheapOSGuide.modelHealth(m))}${m.reasoning===true?' · reasoning advertised':''}</span>${m.health?.last_error?`<small>${esc(m.health.last_error)}</small>`:''}</div>`).join('')||'<p class="small muted">No public-free or explicitly included remote models advertising tool support are listed in this catalog.</p>';
