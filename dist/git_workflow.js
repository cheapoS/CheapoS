...
function completionMarkup(task){
 ...
 return `<section class="chat-result" aria-label="Merged pull request"><div><strong>${esc(summary.title)}</strong><p>${esc(summary.description)}</p><a class="subtle-button" href="${esc(safeURL(p.url)||'#')}" target="_blank" rel="noopener noreferrer">View merged PR #${Number(p.number)}</a>${needsSync(p)?'<button type="button" class="subtle-button" data-pr-sync>Retry sync</button>':''}<p class="small muted">You can archive this task or keep chatting. Start a new task for further changes.</p></div></section>`;
}
...
function content(p){
 if(p.url&&p.ci?.state==='merged'){
  ...
  return `<h3>${esc(summary.title)}</h3><p role="status">${esc(summary.description)}</p><a class="subtle-button" href="${esc(safeURL(p.url)||'#')}" target="_blank" rel="noopener noreferrer">View merged PR #${Number(p.number)}</a>${needsSync(p)||!p.local_sync?'<button type="button" data-pr-refresh>Retry sync</button>':''}<p class="small muted">This is the saved record of the merged work. You can archive this task or keep chatting.</p>`;
 }
...
function mount(container,task,api,onPublished=()=>{}){
 ...
 async function load(status=false){
  ...
  try{current=status?await checkStatus(task,api,{force:true}):await api('/tasks/'+task.id+'/pull-request-preview',{});if(status)onPublished(current);if(!panel.isConnected)return;panel.innerHTML=content(current);bind();
...
const exported={enabled,merged,mergeSummary,completionMarkup,markup,content,mount,safeURL,needsSync,needsStatus,checkStatus,bindSync,freshnessMarkup};if(typeof module!=='undefined')module.exports=exported;else root.CheapOSGitWorkflow=exported;
