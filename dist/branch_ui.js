...
function mount(options){
 ...
 function save(){drafts[key()]={document:documentInput.value,prompt:input.value,overrides:Object.fromEntries([...documentRow.querySelectorAll('[data-planning]')].map(el=>[el.dataset.planning,el.type==='checkbox'?el.checked:el.value]))};try{localStorage.setItem(storageKey,JSON.stringify(drafts));}catch(_){} }
 function sync(){const next=key();if(next!==currentKey){currentKey=next;const draft=drafts[next]||{};documentInput.value=draft.document||'';for(const el of documentRow.querySelectorAll('[data-planning]')){const value=draft.overrides?.[el.dataset.planning];if(el.type==='checkbox')el.checked=value===true;else el.value=value||'';}if(!input.value&&draft.prompt)input.value=draft.prompt;finalPreview=null;summaryHTML='';}documentRow.hidden=false;}
 documentInput.oninput=()=>{save();options.onDraftChange?.();};input.addEventListener('input',save);for(const el of documentRow.querySelectorAll('[data-planning]'))el.addEventListener('input',save);
