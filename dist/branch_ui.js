...
function mount(options){
 ...
 const html=`${workflow.completionMarkup(task)}${startup}${mergeProgressMarkup(task)}${continuationHTML}${pauseMarkup(task,continuation)}${!pause&&p.reason?`<p class="branch-run-reason">${escape(p.reason.replace(/_/g,' '))}</p>`:''}<div class="branch-actions">${['merged','left_on_branch'].includes(p.status)?'<button type="button" data-new-chat>Start a new chat</button>':''}${!pause&&!p.planning&&(['paused','blocked'].includes(p.status)||p.pendingMerge&&p.status!=='merging')&&!reviewAction(task)?`<button type="button" class="primary-button" data-resume>${p.pendingMerge?'Finish saved integration':'Resume run'}</button>`:''}${!pause&&!p.planning&&p.canRecheck&&reviewAction(task)?'<button type="button" data-recheck-run>Recheck changes</button>':''}${!p.planning&&reviewAction(task)?`<button type="button" data-preview>${escape(reviewAction(task).label)}</button>`:''}${p.status==='awaiting_authorization'&&(unfinishedStartup||(!savedStartup&&!['pending','unknown','accepted'].includes(starting?.status)))?(task.branch_run.authorization_ref?'<button type="button" data-finish-startup>Finish saved startup</button>':'<button type="button" data-proposal>Review &amp; start</button>'):''}</div><p class="branch-error" role="alert"></p>`;
