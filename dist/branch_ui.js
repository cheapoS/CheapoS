...
function startController({api,onChange=()=>{},onTask=()=>{},transition=()=>{}}){
 ...
 record.promise=(async()=>{try{const task=await api('/tasks/'+id+'/branch-start',{proposal_id:captured.proposal_id,approved:true});record.status='accepted';if(task?.id===id)onTask(task);onChange(id);}catch(error){record.error=error.message||String(error);await reconcile(id);}return record;})();return record.promise;
 }
...
function mount(options){
 ...
 function settings(){const state=getState(),overrides=Object.fromEntries([...documentRow.querySelectorAll('[data-planning]')].map(el=>[el.dataset.planning,el.type==='checkbox'?el.checked:el.value.trim()]));return {repository:state.project?.path||state.task?.source,prompt:input.value.trim(),document:documentInput.value.trim(),...overrides,feature_ref:overrides.feature_ref||'refs/heads/feature/job-'+Date.now().toString(36),limits:proposedLimits(state.preferences?.limits||{})};}
 async function submitPlanning(retry){
  ...
  const selection=options.openPlanningChat?.();const payload={...values,planning_id:values.planning_id||(globalThis.crypto?.randomUUID?.()||String(Date.now()))};startState={request:payload,selection,draftSignature,error:null};renderStart();options.onDraftChange?.();
  try{
 ...
 function showProposal(existingDialog){
  ...
  const edited=()=>{validation.edit();start.disabled=true;validate.hidden=false;validate.disabled=validation.get().pending;status.textContent='Changes need validation. Start is disabled until this displayed proposal is current.';};
  ...
  form.onsubmit=e=>{e.preventDefault();if(!validation.get().canStart||readiness.blocked)return;guarded(start,async()=>{const exact=validation.get().current,submittedInput=input.value,submittedKey=key();d.close();const result=await starts.start(exact);if(result.status==='accepted'){if(key()===submittedKey&&input.value===submittedInput){delete drafts[submittedKey];try{localStorage.setItem(storageKey,JSON.stringify(drafts));}catch(_){}input.value='';}if(proposal===captured)proposal=null;}await refresh();},d);};
 }
