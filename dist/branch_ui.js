...
'use strict';
const workflow=typeof module==='object'&&module.exports?require('./git_workflow.js'):globalThis.CheapOSGitWorkflow;
const integration=typeof module==='object'&&module.exports?require('./integration.js'):globalThis.CheapOSIntegration;
const schedules=typeof module==='object'&&module.exports?require('./schedules.js'):globalThis.CheapOSSchedules;
const escape=value=>String(value??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
function checkText(value){
 const command=typeof value==='string'?value:value?.command||value?.argv||[];
 const text=typeof command==='string'?command:command.map(s=>/^[\w@%+=:,./-]+$/.test(s)?s:`'${s.replace(/'/g,`'\"'\"'`)}'`).join(' ');
 const directory=value?.check_directory??(typeof value==='object'&&!String(value.directory||'').startsWith('/')?value.directory:'.');
 return directory&&directory!=='.'?`[${directory}] ${text}`:text;
}
function parseChecks(text){return text.split('\n').filter(s=>s.trim()).map(s=>{const m=s.match(/^\[([^\]]+)\] (.+)$/);return m?{command:m[2],directory:m[1]}:s;});}
...
function isBusy(task){return Boolean(task&&(task.branch_run?task.branch_run.startup?.status==='running'||['running','finalizing','merging'].includes(task.branch_run.status)||(task.branch_run.status==='draft'&&['running','waiting_retry'].includes(task.status))||task.status==='stopping':['running','reviewing','waiting_approval','waiting_retry','stopping'].includes(task.status)));}
...
function pausePresentation(task) {
 ...
 const retryPlanning=isPlanning(task)&&['models','inspect','correction','reviewer'].includes(d.next_action);
 const action=run.merge_operation||d.cause==='repeated_review_dispute'||d.next_action==='environment'||retryPlanning?'resume':d.next_action;
 ...
 return {headline:d.cause==='repeated_work'&&d.role==='worker'?'Worker could not choose the next step after recovery.':names[d.cause]||names.unknown,explanation:d.cause==='repeated_review_dispute'?'Review is unfinished. The findings and saved evidence are retained.':String(d.explanation||'Inspect the retained diagnostic.'),saved,action:actions[action]?action:'inspect',actionLabel:run.merge_operation?'Finish saved integration':action==='correction'&&d.cause==='repeated_work'?'Review saved work and continue in chat':actions[action]||actions.inspect,details,question:d.cause==='essential_clarification'?String(run.waiting_for_user||''):''};
}
function reviewAction(task){
 const run=task?.branch_run;
 // Completed items alone do not provide a final diff or review evidence.
 if(!run?.readiness||!['ready_for_merge','paused','blocked','left_on_branch','merged'].includes(run.status))return null;
 return {label:workflow.merged(task)?'View merged changes':run.readiness.manifest?.files?.length?'Review changes':'Review results'};
}
function pauseMarkup(task,continuation){
 const pause=pausePresentation(task);if(!pause)return '';
 const canResume=Boolean(task.branch_run.authorization_ref&&!isPlanning(task)&&!task.pending_approval);
 const resumeFirst=canResume&&['models','inspect','correction'].includes(pause.action);
 const pending=continuation?.status==='pending';
 const primary=resumeFirst?`<button type="button" class="primary-button" data-resume ${pending?'disabled':''}>Resume saved work</button>`:`<button type="button" class="primary-button" data-pause-action ${pending?'disabled':''}>${escape(pause.actionLabel)}</button>`;
 const secondary=resumeFirst?`<button type="button" class="subtle-button" data-pause-action>${escape(pause.actionLabel)}</button>`:'';
 return `<section class="branch-pause" aria-label="Paused work"><strong>${escape(pause.headline)}</strong><p>${escape(pause.explanation)} ${escape(pause.saved)}</p>${pause.question?`<p>${escape(pause.question)}</p>`:''}<div class="button-row">${primary}</div><details data-pause-details><summary>Pause details</summary><ul>${pause.details.map(line=>`<li>${escape(line)}</li>`).join('')}</ul><div class="button-row">${secondary}<button type="button" class="subtle-button" data-view-logs>View technical logs</button></div></details></section>`;
}
function projectRun(task){
 ...
 return {planning:isPlanning(task),measurement:run.plan?.measurement===true,id:run.id,status:run.status,label:workflow.merged(task)?'Merged on GitHub':run.startup?.status==='running'?'Starting':run.status==='awaiting_authorization'&&run.startup?.status==='paused'?'Startup paused':run.status==='awaiting_authorization'&&run.startup?.status==='failed'?'Startup needs attention':isPlanning(task)&&isBusy(task)?'Planning':isPlanning(task)&&/handoffs were tried/.test(task.error||'')?'Planning stopped':isPlanning(task)&&run.pause_reason==='missing_information'?'Needs your reply':run.status==='merged'&&run.merge_receipt?.stage!=='completed'?'Confirming integration':labels[run.status]||'Status unavailable',branch:short(run.feature_ref),target:short(run.target_ref),items,milestones,current:current<0?null:current+1,currentTitle:current<0?'':items[current].title,currentStage:current<0?'':items[current].status,total:items.length,activeSeconds:Number(run.consumption?.working_seconds||0),reason:task.error||run.pause_reason||'',ready:run.status==='ready_for_merge',pendingMerge:Boolean(run.merge_operation),canRecheck:!run.merge_operation&&items.length>0&&items.every(i=>i.done)&&(['ready_for_merge','paused','blocked'].includes(run.status)||(run.status==='finalizing'&&task.status==='paused')),merged:workflow.merged(task)||run.status==='merged'&&run.merge_receipt?.stage==='completed'};
}
function proposalStartBlocked(proposal, allowCommands) {
 const value=proposal.readiness;
 if(!value||value.ready===true&&!(value.checks||[]).some(c=>c.status!=='ready'))return false;
 const checks=Array.isArray(value.checks)?value.checks:[];
 return !allowCommands||!checks.length||checks.some(c=>c.status!=='ready'&&!String(c.id||'').startsWith('command-'));
}
function proposalReadiness(proposal={}) {
 ...
 return {blocked,html:`<section class="branch-readiness" aria-label="Unattended readiness"><strong>${blocked?'Setup to prepare':'Ready to start unattended'}</strong><ul>${checks.map(c=>`<li><strong>${escape(c.label)}</strong> · ${c.status==='ready'?'Ready':String(c.id||'').startsWith('command-')?'Setup needed':'Blocked'}${c.detail?`<br>${escape(c.detail)}`:''}</li>`).join('')}</ul>${blocked?`<p role="alert">${escape(blockers.length?blockers.map(c=>c.detail||c.label).join(' '):'Readiness has not been confirmed. Prepare a refreshed proposal before Start.')}</p>`:''}${assumptions.length?`<h4>Working assumptions</h4><ul>${assumptions.map(a=>`<li>${escape(a)}</li>`).join('')}</ul>`:''}${value.policy?`<p>${escape(value.policy)}</p>`:''}<p>Start authorizes reads and writes in the task copy and this task’s required checks. Select Allow task commands on Review &amp; Start to let the worker prepare dependencies and diagnose failures.</p></section>`};
}
...
function finalReviewMarkup(task,preview){
 ...
 return `${workflow.enabled(task)?workflow.markup(task):integration.markup(task,preview.integration_readiness)}<div class="review-overview"><p class="review-task-title">${escape(task.title||'Saved branch changes')}</p><p class="review-branch-route"><strong>${escape(short(task.branch_run.feature_ref))}</strong><span>→</span><strong>${escape(short(preview.target_ref||task.branch_run.target_ref))}</strong><span>${workflow.enabled(task)?'GitHub pull request':'Local merge · no push'}</span></p>${preview.destination&&!workflow.enabled(task)?`<p class="review-destination">Merge location: <code>${escape(preview.destination)}</code></p>`:''}<div class="review-summary"><span>${files.length} files</span><span class="review-add">+${total('added_lines')}</span><span class="review-remove">−${total('removed_lines')}</span><button type="button" data-evidence aria-expanded="false">${checks.length?`${passed}/${checks.length} checks passed`:'Check evidence unavailable'} · ${review?.decision==='APPROVE'?'Final review approved':'Inspect final review'}</button></div><p class="review-blocker" role="status" ${preview.blocker?'':'hidden'}>${escape(preview.blocker)}</p></div>
...
function technicalPage(task,anchor=null){
 const saved=task?.events||[],index=anchor?saved.findIndex(e=>String(e.id)===String(anchor)):-1;
 const end=index<0?saved.length:index+1,start=Math.max(0,end-100),newerEnd=Math.min(saved.length,end+100);
 return {events:saved.slice(start,end).reverse(),total:saved.length,first:saved.length-end+1,last:saved.length-start,
   older:start>0?String(saved[start-1].id):null,newer:end<saved.length?(newerEnd===saved.length?'':String(saved[newerEnd-1].id)):null};
}
...
function technicalMarkup(task,anchor=null){
 const page=technicalPage(task,anchor),events=page.events,pause=pausePresentation(task);
 const navigation=page.total>100?`<nav class="button-row" aria-label="Technical log pages"><button class="outline-button" data-log-page="" ${page.newer===null?'disabled':''}>Latest events</button><button class="outline-button" data-log-page="${escape(page.newer||'')}" ${page.newer===null?'disabled':''}>Newer events</button><span>Events ${page.first}–${page.last} of ${page.total}</span><button class="outline-button" data-log-page="${escape(page.older||'')}" ${page.older===null?'disabled':''}>Older events</button></nav>`:'';
 ...
 const fields=['error','error_code','code','attempt','reason','failure_reason','stage','role','model','requested_model','selected_model','actual_model','request_id','diagnostic_id','exit_code','status','summary','message','command','output','retry_at','cooldown_scope'];
...
function startController({api,onChange=()=>{},onTask=()=>{},transition=()=>{}}){
 ...
 record.promise=(async()=>{try{const task=await api('/tasks/'+id+'/branch-start',{proposal_id:captured.proposal_id,approved:true,...(typeof captured.allow_task_commands==='boolean'?{allow_task_commands:captured.allow_task_commands}:{}),...(captured.full_suite_approved===true?{full_suite_approved:true}:{}),...(captured.schedule?{schedule:captured.schedule}:{})});record.status='accepted';if(task?.id===id)onTask(task);onChange(id);}catch(error){record.error=error.message||String(error);await reconcile(id);}return record;})();return record.promise;
 }
...
function mount(options){
 ...
 let scheduleRequest=null;let currentKey=null,busy=false,busySelection=null,interactiveOnce=false,proposal=null,summaryHTML='',activeDialog=null,startState=null;const detailStates=new Map();
 ...
 let scheduleMarkup='';const scheduleRow=document.createElement('div');scheduleRow.className='schedule-intent';scheduleRow.hidden=true;controls.after(scheduleRow);
 function syncSchedule(){
  const task=getState().task;
  const info=schedules.status(task)||(!task&&selector.value==='unattended'&&scheduleRequest?{...scheduleRequest,label:'Awaiting approval'}:null);
  scheduleRow.hidden=!info;
  if(!info){scheduleRow.innerHTML='';scheduleMarkup='';return;}
  const html=`<strong>${info.interval_hours?escape(schedules.frequency(info.interval_hours))+' · ':''}${escape(info.label)}</strong>${!task?`<label>Repeat<select aria-label="Schedule frequency">${schedules.intervalOptions(info.interval_hours,true)}</select></label>`:''}${info.manage?'<button type="button" data-manage-schedule>Manage schedule</button>':!task?.branch_run?.authorization_ref?'<span>Review the plan to approve the first run and recurrence.</span>':''}`;
  // Compare our markup, not browser-normalized innerHTML, to retain focus on polls.
  if(scheduleMarkup===html)return;
  scheduleMarkup=html;scheduleRow.innerHTML=html;
  const frequency=scheduleRow.querySelector('select');
  if(frequency)frequency.onchange=()=>{scheduleRequest=Number(frequency.value)?{interval_hours:Number(frequency.value)}:null;save();syncSchedule();};
  const manage=scheduleRow.querySelector('[data-manage-schedule]');
  if(manage)manage.onclick=()=>options.manageSchedules?.();
 }
 ...
 function save(){drafts[key()]={schedule_request:selector.value==='unattended'?scheduleRequest:null,mode:selector.value,document:documentInput.value,prompt:input.value,overrides:Object.fromEntries([...documentRow.querySelectorAll('[data-planning]')].map(el=>[el.dataset.planning,el.type==='checkbox'?el.checked:el.value]))};try{localStorage.setItem(storageKey,JSON.stringify(drafts));}catch(_){} }
 function sync(){const next=key();if(next!==currentKey){currentKey=next;const draft=drafts[next]||{};scheduleRequest=draft.schedule_request||null;selector.value=draft.mode==='unattended'?'unattended':'interactive';documentInput.value=draft.document||'';for(const el of documentRow.querySelectorAll('[data-planning]')){const value=draft.overrides?.[el.dataset.planning];if(el.type==='checkbox')el.checked=value===true;else el.value=value||'';}if(!input.value&&draft.prompt)input.value=draft.prompt;summaryHTML='';}const run=getState().task?.branch_run;selector.disabled=hasRun(getState().task);if(selector.disabled)selector.value='unattended';documentRow.hidden=selector.value!=='unattended'||selector.disabled;syncSchedule();}
 ...
 function settings(){const state=getState(),overrides=Object.fromEntries([...documentRow.querySelectorAll('[data-planning]')].map(el=>[el.dataset.planning,el.type==='checkbox'?el.checked:el.value.trim()]));return {repository:state.project?.path||state.task?.source,prompt:input.value.trim(),document:documentInput.value.trim(),...(scheduleRequest?{schedule_request:scheduleRequest}:{}),...overrides,feature_ref:overrides.feature_ref||'refs/heads/feature/job-'+Date.now().toString(36),uncapped_work:state.preferences?.limits?.uncapped_work===true,limits:proposedLimits(state.preferences?.limits||{})};}
 ...
 const commandText=checkText;
  ...
  function showProposal(existingTarget,initialTab='tasks'){
   const allowCommands=existingTarget?.querySelector('[name=allow_task_commands]')?.checked??true;
   ...
   const body=`<div class="branch-review-body plan-proposal-view">
    <div class="proposal-header">
      <div class="proposal-header-main">
        <h2>${captured.schedule_preview?'Review &amp; start scheduled task':'Review &amp; start unattended run'}</h2>
    ...
    <form>
      ...
      <section class="proposal-tab-panel hidden" data-tab-panel="checks">
        ...
        <div class="proposal-card"><details class="review-permissions" open><summary>Required verification commands</summary><ul>${(contract.check_scope||[]).map(scope=>`<li><code>${escape(commandText(scope))}</code>${scope.kind?` · ${escape(scope.kind)}`:''}</li>`).join('')||'<li>Inspect the required checks above. Complete validated command scopes must be available before Start.</li>'}</ul><p class="small muted">These checks remain required even when the worker prepares dependencies or runs diagnostics. Feature commits are local; merge and push remain separate decisions.</p></details></div>
      ...
      <section class="proposal-tab-panel hidden" data-tab-panel="start">
        <div class="proposal-card"><label class="checkbox-field"><input type="checkbox" name="allow_task_commands" ${allowCommands?'checked':''}><span>Allow task commands · recommended for unattended work</span></label><p>Let the worker install dependencies and run setup, diagnostics, builds and tests from this task copy without asking for each command. This permission stays with this task across restarts and can be revoked in Session permissions. Required checks and independent review still apply.</p><p class="small muted">Commands and dependency scripts run with your user account’s permissions. The task copy is not a security sandbox. Deployment, publishing, credential access and edits to other checkouts are outside this permission.</p></div>
        ...
        ${schedules.approvalMarkup(captured.schedule_preview,captured.full_suite_checks)}
        ${fullSuiteConsent(captured) || ''}
        <div data-review-readiness>${readiness?.html || ''}</div>
        <p data-review-status role="status">${readiness.blocked?'The worker can prepare missing prerequisites when task commands are allowed. An unavailable task copy still needs attention.':'This proposal is current and ready for your review.'}</p>
        ...
        <div class="proposal-step-nav proposal-actions-row">
          ...
          <div class="branch-actions">
            ...
            <button type="submit" class="primary primary-button start-run-btn" ${readiness.blocked?'disabled':''}>${captured.schedule_preview?'Approve &amp; start schedule':'Start run'}</button>
          </div>
        </div>
      </section>
    </form>
   ...
   const repeat=form.elements.schedule_interval;
   if(repeat)repeat.onchange=()=>{form.querySelector('.branch-error').textContent='';form.elements.schedule_approved.checked=false;if(form.elements.schedule_full)form.elements.schedule_full.checked=false;form.querySelector('[data-recurring-consent]').hidden=repeat.value==='0';start.textContent=repeat.value==='0'?'Start run':'Approve & start schedule';};
   const setupBlocked=()=>proposalStartBlocked(captured,form.elements.allow_task_commands.checked);
   const validation=proposalValidation(captured,values=>api('/tasks/'+captured.task_id+'/branch-proposal-edit',values));
   start.disabled=!validation.get().canStart||setupBlocked();
   const edited=event=>{if(['full_suite_approved','allow_task_commands','schedule_interval','schedule_approved','schedule_full'].includes(event?.target?.name)){start.disabled=!validation.get().canStart||setupBlocked();return;}validation.edit();start.disabled=true;validate.hidden=false;validate.disabled=validation.get().pending;status.textContent='Changes need validation. Start is disabled until this displayed proposal is current.';};
   ...
   validate.onclick=async()=>{
    ...
    const next=structuredClone(plan);next.items.forEach((item,i)=>{item.title=form.querySelector(`[data-title="${i}"]`).value;item.instructions=form.querySelector(`[data-instructions="${i}"]`).value;item.acceptance_criteria=form.querySelector(`[data-criteria="${i}"]`).value.split('\n').filter(x=>x.trim());item.required_checks=parseChecks(form.querySelector(`[data-checks="${i}"]`).value);});next.final_checks=parseChecks(form.querySelector('[data-final-checks]').value);next.measurement=form.elements.measurement.checked;next.uncapped_work=form.elements.uncapped_work.checked;for(const el of form.querySelectorAll('[data-limit]'))next.limits[el.dataset.limit]=Number(el.value);
   ...
   form.onsubmit=e=>{e.preventDefault();if(!validation.get().canStart||setupBlocked())return;if((captured.full_suite_checks||[]).length&&!form.elements.full_suite_approved?.checked){error(form,new Error('Approve the listed full-suite checks before starting, or revise the checks.'));return;}guarded(start,async()=>{const schedule=schedules.approval(form,captured.schedule_preview,captured.full_suite_checks);const exact={...validation.get().current,...(schedule?{schedule}:{}),allow_task_commands:form.elements.allow_task_commands.checked,...(form.elements.full_suite_approved?.checked?{full_suite_approved:true}:{})},submittedInput=input.value,submittedKey=key();if(isDialog)d.close();const result=await starts.start(exact);if(result.status==='accepted'){if(key()===submittedKey&&input.value===submittedInput){delete drafts[submittedKey];try{localStorage.setItem(storageKey,JSON.stringify(drafts));}catch(_){}input.value='';}if(proposal===captured)proposal=null;options.showChat?.();}await refresh();},d);};
  }
 ...
 const html=workflow.merged(task)?workflow.completionMarkup(task):`${startup}${mergeProgressMarkup(task)}${continuationHTML}${pauseMarkup(task,continuation)}${!pause&&p.reason?`<p class="branch-run-reason">${escape(p.reason.replace(/_/g,' '))}</p>`:''}<div class="branch-actions">${['merged','left_on_branch'].includes(p.status)?'<button type="button" data-new-chat>Start a new chat</button>':''}${!pause&&!p.planning&&(['paused','blocked'].includes(p.status)||p.pendingMerge&&p.status!=='merging')&&!reviewAction(task)?`<button type="button" class="primary-button" data-resume>${p.pendingMerge?'Finish saved integration':'Resume run'}</button>`:''}${!pause&&!p.planning&&p.canRecheck&&reviewAction(task)?'<button type="button" data-recheck-run>Recheck changes</button>':''}${!p.planning&&reviewAction(task)?`<button type="button" data-preview>${escape(reviewAction(task).label)}</button>`:''}${p.status==='awaiting_authorization'&&(unfinishedStartup||(!savedStartup&&!['pending','unknown','accepted'].includes(starting?.status)))?(task.branch_run.authorization_ref?'<button type="button" data-finish-startup>Finish saved startup</button>':'<button type="button" data-proposal>Review &amp; start</button>'):''}</div><p class="branch-error" role="alert"></p>`;
 ...
 const resume=panel.querySelector('[data-resume]');if(resume){resume.disabled=!options.resume||continuation?.status==='pending';resume.onclick=()=>guarded(resume,()=>options.resume(task),panel);if(!options.resume)resume.title='Resume needs command permission revalidation';}
 ...
 function renderPlan(task){
  ...
  if(panel.dataset.task!==task.id||!panel.querySelector('[data-plan-content]')){panel.dataset.task=task.id;panel.innerHTML='<div class="plan-review-heading"><div><h2>Plan</h2><p>Follow the approved scope, acceptance criteria, and item progress.</p></div><div class="plan-heading-actions"><button type="button" data-schedule-task hidden>Schedule this task…</button><button type="button" data-jump-changes class="primary-button" hidden></button></div></div><div data-plan-content></div>';panel.querySelector('[data-jump-changes]').onclick=()=>showFinal(task);}
  const schedule=panel.querySelector('[data-schedule-task]');
  if(schedule){schedule.hidden=!options.scheduleTask||!task.branch_run?.authorization_ref||Boolean(task.trashed_at);schedule.onclick=()=>options.scheduleTask(task);}
  const review=reviewAction(task),link=panel.querySelector('[data-jump-changes]');link.hidden=!review;link.textContent=review?review.label+' →':'';
 ...
 function renderChanges(task){
  ...
  const run=task.branch_run,signature=JSON.stringify([run?.readiness?.id,run?.status,run?.expected_feature_tip,run?.merge_progress?.stage,task.pull_request,task.integration_preparation,task.archived_at,task.trashed_at]);
  ...
  if(!run?.readiness){panel.innerHTML=integration.markup(task)+'<div class="empty-state"><h2>Cumulative review is not ready yet.</h2><p>The saved branch diff and merge controls will appear here after final checks and independent review. Follow current work in Chat or item progress in Plan.</p></div>';integration.bind(panel,task,null,api,refresh);return;}
 ...
 async function loadFinal(task,slot){
  ...
  if(workflow.enabled(task)){d.querySelector('[data-merge]').hidden=true;workflow.mount(d,task,api,()=>{slot.dataset.signature='';void refresh();});}
  if(!task.workspace_cleanup&&typeof CheapOSPreview!=='undefined')CheapOSPreview.mount(d.querySelector('.review-overview'),task,api,preview.feature_tip);
  ...
  integration.bind(d,task,preview.integration_readiness,api,async saved=>{
   slot.dataset.signature='';
   if(getState().task?.id===task.id){options.receiveUpdatedTask?.(saved);options.showChat?.();}
   await refresh();
  },()=>options.showChat?.(),saved=>options.resume(saved));
 ...
 sync();return {clearSubmittedDraft:(owner,message)=>{if(drafts[owner]?.prompt?.trim()===message){drafts[owner].prompt='';try{localStorage.setItem(storageKey,JSON.stringify(drafts));}catch(_){}}},isSubmitting:()=>busy&&busySelection===getState().selection,interceptSubmit,render,renderPlan,renderChanges,renderStart,showProposal,showFinal,sync,restoreDraft:()=>{sync();if(!input.value&&drafts[key()]?.prompt)input.value=drafts[key()].prompt;},hasDocument:()=>selector.value==='unattended'&&Boolean(documentInput.value.trim()),getMode:()=>selector.value,newChat:(mode='interactive',schedule=null)=>{sync();scheduleRequest=schedule;selector.value=mode==='unattended'?'unattended':'interactive';documentInput.value='';delete group.dataset.revising;save();sync();options.onDraftChange?.();}};
}
return {checkText,parseChecks,mergeAndPublish,mergeProgressMarkup,fullSuiteConsent,diffPath,reviewDiffs,reviewFiles,reviewDiffMarkup,finalReviewMarkup,finalDiffState,mountFinalDiff,planningPayload,proposalValidation,technicalEvents,technicalPage,technicalText,technicalMarkup,startController,savedPlan,planMarkup,pausePresentation,pauseMarkup,reviewAction,proposalReadiness,proposalStartBlocked,mount,intent,terminalRun,hasRun,isPlanning,duration,isBusy,isRevisionTarget,resumeAction,proposedLimits,projectRun,diffSections,escape};
});
