...
function markup(task,readiness){
 let op=readiness?.publication_id?null:task.integration_preparation;
 ...
 if(op?.status==='ready')op={...op,reason:undefined,error:undefined};
 ...
 const localNotice=canUpdate&&readiness.local_changes?.length?`<div class="integration-local-notice"><p>Local changes will stay untouched. This update uses the target branch’s committed changes. Unrelated local edits can stay in place when you merge; overlapping edits remain protected.</p><ul>${readiness.local_changes.map(p=>`<li>${esc(p)}</li>`).join('')}</ul></div>`:'';
 ...
 return summary+`<section class="integration-update" aria-label="Integration readiness"><h3>${canUpdate?'Bring this work up to date':esc(readiness.message||'Integration status')}</h3>${readiness.publication_id?`<p>The saved PR destination <strong>${esc(readiness.previous_base)}</strong> no longer exists on GitHub. Prepare the saved work against <strong>${esc(readiness.target_ref?.replace(/^refs\/heads\//,''))}</strong> at <code>${esc(readiness.target_tip?.slice(0,12))}</code>.</p><p>This preserves the previous publication and pushed branch, updates the task copy, and requires fresh checks and independent review. Publishing to this destination requires a separate approval.</p>`:canUpdate?'<p>The project changed while this task was running. cheapoS will combine both versions, check the result, and request fresh review.</p><p class="small muted">This updates the task copy. Final approval is still required to change your project.</p>':''}${localNotice}${checkConsent}${(readiness.files||[]).length?`<ul>${readiness.files.map(p=>`<li>${esc(typeof p==='string'?p:p.path)}</li>`).join('')}</ul>`:''}<div class="button-row">${canUpdate?'<button type="button" class="primary-button" data-integration-prepare>Update &amp; resolve</button>':''}${(readiness.actions||[]).includes('inspect_local_changes')?'<button type="button" data-integration-local>Inspect local changes</button>':''}${canUpdate?`<button type="button" data-integration-keep>${task.branch_run?'Keep on branch':'Keep saved work'}</button>`:''}</div><p class="form-error" role="alert" data-integration-error></p></section>`;
}
function prepare(api,task,readiness){
 const key=[task.id,readiness.target_ref,readiness.target_tip,readiness.candidate,readiness.publication_id].join(':');
 ...
 record.promise=api('/tasks/'+task.id+'/integration-prepare',{approved:true,target_tip:readiness.target_tip,target_ref:readiness.target_ref,candidate:readiness.candidate,operation_id,...(readiness.publication_id?{publication_id:readiness.publication_id}:{})}).catch(async error=>{
