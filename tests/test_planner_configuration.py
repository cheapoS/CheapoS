...
class PlannerConfigurationTests(unittest.TestCase):
    ...
    def test_fallback_credentials_preserve_role_and_endpoint(self):
        ...
        e.config={**e.config, 'planner':validate_provider(dict(self.provider,base_url='https://other.example/v1'),'planner')}
    ...
    def test_dedicated_configuration_roundtrip_and_explicit_reset(self):
        ...
        e.configure({'worker':dict(self.provider, model='fixture-worker'),'reviewer':self.provider,'planner':dict(self.provider,model='strong-planner')})
        ...
        self.assertNotIn('secret-fixture',(e.store.root/'settings.json').read_text())
