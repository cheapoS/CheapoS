...
class BranchCompletionTests(unittest.TestCase):
    ...
    def test_final_preview_is_read_only_and_merge_is_explicit_idempotent(self):
        ...
        (destination/'PROTOCOL.md').write_text('untracked local draft')
        destination_status=git(destination,'status','--porcelain')
        ...
        self.assertEqual(preview['integration_readiness']['local_changes'],['PROTOCOL.md'])
        ...
        self.assertEqual(git(destination,'status','--porcelain'),destination_status)
        self.assertEqual((destination/'PROTOCOL.md').read_text(),'untracked local draft')
        self.assertEqual(git(destination,'ls-tree','HEAD','--','PROTOCOL.md'),'')
        self.assertEqual(git(self.source,'rev-parse','HEAD'),before)
