...
test('receipt counts distinguish independent disproof from human integration',()=>{
 const label=modelHealth({health:{role_evidence:{worker:{completion_samples:2,completed:2,independently_validated:1,independently_disproved:1,human_integrated:1}}}});
 for(const value of ['2 observed completions','1 independently confirmed','1 independently disproved','1 human integrated'])assert.ok(label.includes(value));
