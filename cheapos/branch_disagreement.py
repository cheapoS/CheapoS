...
REVIEW_INSTRUCTION = (' REQUEST_CHANGES requires 1–8 defects with an exact criterion, precise code/evidence location, '
    'expected and observed behavior, kind (static or executable), support explaining the evidence, and reproduction '
    '(required for executable findings; empty for static findings). Distinguish a proposed reproduction from a result '
    'actually observed. Explain why supplied passing checks miss the defect. Do not prescribe a rewrite based on an '
    'unsupported assumption. A static defect may cite a precise code path and reasoning without executing anything.')
...
def before_write(task, path):
    ...
    if ('..' not in name.parts and not name.is_absolute() and
        ...
        return
    raise ValueError('Demonstrate the executable review claim first: add a narrow test and run an approved check before editing implementation. A passing counterprobe can go to checkpoint without changing correct behavior. New commands still require ordinary approval.')
