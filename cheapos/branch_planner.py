...
def _parse(message, limits, source=None, assumptions=None):
    ...
    for item in result['items']:
        filtered = [c for c in item.get('required_checks', []) if not is_plan_preview(c)]
        if filtered:
            item['required_checks'] = filtered
        elif item.get('required_checks'):
            raise ValueError("items[%s].required_checks: check.py --plan lists checks but runs none; specify executable test commands" % item['id'])
    filtered_final = [c for c in result.get('final_checks', []) if not is_plan_preview(c)]
    if filtered_final:
        result['final_checks'] = filtered_final
    elif result.get('final_checks'):
        raise ValueError("final_checks: check.py --plan lists checks but runs none; specify executable test commands")
