...
import copy
import unittest
from contract import task, has, private_absent, commit_present, commit_absent, unavailable, escaped, SHA
...
class FormatterAcceptance(unittest.TestCase):
    ...
    def test_real_schema_determinism_allowlist_and_example(self):
        ...
        self.assertRegex(report.lower(), r'check[^\n]*2|2[^\n]*check')
        self.assertRegex(report.lower(), r'1[^\n]*fail|fail[^\n]*1')
    ...
    def test_outcomes_and_merge_receipt_identity(self):
        ...
        self.assertRegex(missing.lower(), r'unconfirmed|confirming|unavailable|incomplete|pending|not.confirmed')
        ...
        self.assertRegex(confirmed.lower(), r'merged|integrated')
        for field, wrong in (('mapping', {'run_id':'other'}), ('feature_tip', 'c3'*20), ('target_ref', 'refs/heads/other'), ('stage', 'intent')):
            ...
            self.assertRegex(self.render(value).lower(), r'unconfirmed|confirming|unavailable|incomplete|pending|not.confirmed')
    ...
    def test_missing_partial_zero_and_uncertain_accounting(self):
        ...
        self.assertNotRegex(report.lower(), r'reviewer[^\n]*tokens[^\n]*[:|]\s*0\b')

    def test_markdown_unicode_order_and_typed_pause(self):
        value = task(); run = value['branch_run']; value['title'] = '東京 |raw|\n# injected'
