...
def main():
 ...
 try:
  ...
  independent={'source_main_unchanged':git(source,'rev-parse','main').strip()==baseline,'source_clean':git(source,'status','--porcelain')=='','tests_unchanged':(Path(d['workspace'])/'test_acceptance.py').read_bytes()==(source/'test_acceptance.py').read_bytes()}
