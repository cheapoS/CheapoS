...
Failed probes move to another free candidate. For transient HTTP failures, connection failures, broken response JSON, incomplete streams, or calls to tools not offered in that step, an automatic remote chat can hand off to a different free model, at most twice per run across worker and reviewer. Each replacement selection checks up to four candidates, excludes failed models from this run, and keeps the other role distinct. The same saved conversation and tool results go to the replacement; failed responses never execute partial tool calls. Existing worker-turn, checkpoint-turn, time, dollar, and reviewer-token limits remain in force. User stops, authentication/credit errors, refusals, and missing token usage in otherwise completed responses pause instead. Failed review recovery preserves the checkpoint and reuses matching passing checks. Existing failed automatic chats select an alternative when explicitly resumed; server startup does not resume tasks. cheapoS never substitutes local work or a paid route automatically. Unexpected cost reports below $0.01 across the task are tolerated within its explicit dollar budget. At $0.01 cumulative accounted cost, automatic free/included routing stops before returning tools or dispatching another request. Retries, model handoffs and restarts do not reset this cutoff; every reported charge remains counted. A lower task budget, including $0, still stops work when exceeded. Catalog prices and a zero-dollar cap are not provider-side billing guarantees: inspect OmniRoute's own fallbacks and billing settings.

When an automatic worker reaches its response output cap, cheapoS retries once per model per user request with guidance for one smaller complete action, using the configured output and work policy. Supported reasoning adjustments apply only to that retry. A complete response ends the temporary guidance; the saved attempt still counts, so repeated truncation on the same model leads to an authorized worker handoff rather than renewing that retry. Manual/local selections retain their existing authority. Both streamed and JSON responses discard all tool calls on `finish_reason=length`; reported usage is counted, or the conservative reservation remains when unavailable. Recovery preserves spending and command permissions, verification and independent review.

Coherent edits are accepted by default. `replace_lines` sends only the new text for an inclusive line range and checks the controller-held file version before writing; it has no fixed line-count limit. Existing text tools retain their 2,000,000-byte UTF-8 file ceiling, and new files can contain up to 256,000 bytes. The former 80-line / 3,000-byte edit cap and separate 24,000-byte creation cap no longer apply. Existing files cannot be overwritten by `write_file`.

Malformed edit arguments trigger temporary smaller-edit guidance with current numbered evidence. All normal edit tools remain available. A successful edit or a change of worker, connection, item, baseline, or user request clears that guidance. Resume preserves an unresolved same-worker attempt. Output-truncation guidance clears after a complete response; historical retry attempts remain recorded. No extra work allowance, automatic command approval, or review bypass is granted. See [worker policy](development/work-policy.md).
